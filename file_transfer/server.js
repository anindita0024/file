...
function recursiveGetFile (files, _id) {
    ...
    for (const a = 0; a < files.length; a++) {
        const file = files[a];
...
function recursiveSearchShared (files, query) {
    ...
    for (const a = 0; a < files.length; a++) {
        const file = (typeof files[a].file === "undefined") ? files[a] : files[a].file;
...
app.listen(3000,  () => {
    console.log("Server running on  " + mainURL);

    // connect with mongo DB server
    mongoClient.connect("mongodb+srv://file:1234@cluster0.fr8fe.mongodb.net/myFirstDatabase?retryWrites=true&w=majority", {
        ...
        app.get("/SharedWithMe/:_id?", async function (request, res) {
            res.render("SharedWithMe", {
                "request": request
            });
        });

        app.post("/DeleteLink", async  (request, res) => {
        ...
        app.get("/MySharedLinks", async  (request, res) => {
            if (request.session.user) {
                const links = await database.collection("public_links").find({
                    "uploadedBy._id": ObjectId(request.session.user._id)
                }).toArray();
        ...
        app.get("/SharedViaLink/:hash", async  (request, res) => {
        ...
        app.post("/ShareViaLink", async  (request, res) => {
            const _id = request.fields._id;
        ...
        app.post("/DeleteFile", async  (request, res) => {
            const _id = request.fields._id;
            ...
            if (request.session.user) {
                ...
                const updatedArray = await removeFileReturnUpdated(user.uploaded, _id);
                for (const a = 0; a < updatedArray.length; a++) {
        ...
        // download file
        app.post("/DownloadFile", async  (request, res) => {
            const _id = request.fields._id;
            ...
            if (link != null) {
                fileSystem.readFile(link.file.filePath,  (error, data) => {
                    // console.log(error);
        ...
        app.get("/MyUploads", async  (request, res) => {
            if (request.session.user) {
        ...
        // upload new file
        app.post("/UploadFile", async  (request, res) => {
            if (request.session.user) {
                ...
                if (request.files.file.size > 0) {
                    ...
                    const filePath = "public/uploads/" + user.email + "/" + new Date().getTime() + "-" + request.files.file.name;
